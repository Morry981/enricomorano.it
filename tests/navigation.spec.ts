...
const pages = [
    ...
    { path: '/skills',    titlePattern: /Competenze/,        heading: 'Le mie Competenze' },
...
test.describe('Navigazione tra pagine', () => {
    for (const { path, titlePattern, heading } of pages) {
        test(`${path} — titolo e heading corretti`, async ({ page }) => {
            ...
            await expect(page.getByRole('heading', { level: 1 })).toContainText(heading.replace('Io sono Enrico', 'Enrico'));
        });
    ...
    test('navigazione da home a Progetti via nav', async ({ page }) => {
        await page.goto('/');
        await page.locator('nav').getByRole('link', { name: 'Progetti' }).click();
        await expect(page).toHaveTitle(/Progetti/);
        await expect(page.locator('nav a[aria-current="page"]')).toHaveAttribute('href', '/projects');
    });

    test('navigazione da home a Chi Sono via nav', async ({ page }) => {
        await page.goto('/');
        await page.locator('nav').getByRole('link', { name: 'Chi Sono' }).click();
